...
class TestRichConsoleReporter:
    def test_reporter_as_listener(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        tracker.record_iteration(
            ...
            strategy_id="s1",
            strategy_name="test-strat",
            shape_results={"small": 0.8},
            vs_baseline={"small": 20.0},
            compile_ok=True,
            correctness_ok=True,
            wall_time_s=3.0,
        )
        tracker.emit_final(
            overall_speedup=1.25,
            regime_winners=[{"id": "s1", "name": "test-strat", "regime": "all", "latency_ms": "0.800"}],
            pruned_strategies=[],
            total_iterations=1,
        )
...
class TestHTMLReportGenerator:
    def test_generate_creates_file(self, tmp_path: Path) -> None:
        ...
        tracker.record_iteration(
            ...
            strategy_id="s1",
            strategy_name="test-strat",
            shape_results={"small": 0.7, "medium": 3.5},
            vs_baseline={"small": 30.0, "medium": 30.0},
            compile_ok=True,
            correctness_ok=True,
            wall_time_s=3.0,
        )
        tracker.emit_final(
            overall_speedup=1.5,
            regime_winners=[{"id": "s1", "name": "test-strat", "regime": "all", "latency_ms": "3.500"}],
            pruned_strategies=[],
            total_iterations=1,
        )
    ...
    def test_report_contains_charts(self, tmp_path: Path) -> None:
        ...
        for i in range(3):
            tracker.record_iteration(
                ...
                strategy_id="s1",
                strategy_name="test",
                shape_results={"small": 0.5 - i * 0.05, "medium": 2.0 - i * 0.2},
                vs_baseline={"small": i * 10.0, "medium": i * 10.0},
                compile_ok=True,
                correctness_ok=True,
                wall_time_s=2.0,
            )
